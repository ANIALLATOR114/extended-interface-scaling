...
parser.add_argument("-n", "--name", default="mod name", help="Mod name")
parser.add_argument("-v", "--version", default="1.0.0", help="Mod version")
parser.add_argument("-d", "--description", default="", help="Mod description")
    ...
    for elem in os.listdir(folder):
        ...
        if os.path.isfile(full_path):
            if os.path.splitext(full_path)[1] == ".py":
                compile(full_path)
                file.write(
                    full_path + "c", "./res" + full_path.split("res", 1)[1] + "c"
                )
